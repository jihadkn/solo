...
import (
	...
	"github.com/jihadkn/etc-core-solo/rpc"
	"github.com/jihadkn/etc-core-solo/storage"
	"github.com/jihadkn/etc-core-solo/util"
)
...
// Donate 0.1% from pool fees to developers
const donationFee = 0.1
const donationAccount = "0x03d59d4d67d618d9aa692ce20012acbf04b471fa"
...
func (u *BlockUnlocker) handleBlock(block *rpc.GetBlockReply, candidate *storage.BlockData) error {
  ...
  rewardForUncles := big.NewInt(0).Mul(uncleReward, big.NewInt(int64(len(block.Uncles))))	reward.Add(reward, rewardForUncles)
